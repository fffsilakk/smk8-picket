...
<template>
  <AdminPage>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        <h2 class="text-2xl font-semibold mb-6">Tambah Jurusan</h2>
        <form @submit.prevent="addData" >
          <div class="mb-4">
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
              Nama Jurusan
            </label>
            <input v-model="data.formData.name" type="text" id="name"
              ...
              placeholder="name" />
            <LabelError v-if="data.errors.length > 0" :errors="data.errors" propName="Name"></LabelError>
          </div>

          <div class="mb-4">
            <label for="initial" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
              Inisial
            </label>
            <input v-model="data.formData.initial" id="initial" type="text" placeholder="Inisial"
          ...
          <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
            <div class="px-4 py-2 bg-white rounded-t-lg dark:bg-gray-800">
              ...
              <div>
                <textarea v-model="data.formData.description" id="description" rows="4"
                  class="w-full px-0 text-sm text-gray-900 bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white dark:placeholder-gray-400"
                  placeholder="Write a comment..."></textarea>
              </div>
              <LabelError v-if="data.errors.length > 0" :errors="data.errors" propName="Description"></LabelError>
            </div>
          </div>
