...
const addTeacher = async () => {
  try {
    await axios.post("https://picket.ocph23.tech/api/teacher", form.value, {
...
<template>
  <div class="mt-12 pt-10 p-6 md:px-10 sm:px-6 px-4 md:ml-64 sm:ml-64 ml-20">
    ...
    <form @submit.prevent="addTeacher" class="space-y-4">
      <div>
        <label for="number" class="block text-gray-700">Nomor</label>
        <input
          id="number"
          v-model="form.number"
          type="text"
      ...
      <div>
        <label for="grnder" class="block text-gray-700">Jenis Kelamin</label>
